...
@asyncio.coroutine
def logger_factory(app, handler): #日志记录器工厂协程
    @asyncio.coroutine
    def logger(request): #日志记录器协程
...
@asyncio.coroutine
def auth_factory(app, handler): #认证工厂协程
    @asyncio.coroutine
    def auth(request): #认证协程
...
loop = asyncio.get_event_loop() #获得时间循环对象
loop.run_until_complete(init(loop)) #放入初始化的循环对象，并运行直至完成
